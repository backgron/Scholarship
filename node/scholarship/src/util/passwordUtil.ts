...
//判断密码是否正确
export async function isPassword(user: (Admin | Student | Counselor), model: Model<Admin | Student | Counselor>, password: string) {
  console.log(user)
...
//修改密码
export async function changePassword(user: (Admin | Student | Counselor), model: Model<Admin | Student | Counselor>, pre: string, next: string) {
  //判断密码是否正确或为初始值
